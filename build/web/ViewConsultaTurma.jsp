<%-- 
    Document   : ViewConsultaTurma
    Created on : 05/09/2018, 20:55:08
    Author     : Roberto Oswaldo Klann
--%>

<%@page import="Controller.Turma"%>
<%@page import="Controller.Conexao"%>
<%@page import="Modelo.TurmaModel"%>
            ...
            <table class="table table-striped">
                <tr>
                 ...
                 <%
                    ...
                    ArrayList lista    = Turma.getTurmas(conexao);
                    for(int i=0; i < lista.size(); i++){
                        TurmaModel turma = (TurmaModel) lista.get(i);                
                 %>
                    <tr>
                        <td><%=turma.getCodigoTurma()%></td>
                        <td><%=turma.getCodigoCurso()%></td>
                        <td><%=turma.getCodigoProfessor()%></td>
                        <td><%=turma.getDataFinal()%></td>
                        <td><%=turma.getDataInicial()%></td>
                        <td><%=turma.getHorario()%></td>
                        <td>
                            <a href="excluicidade.jsp?codigo=<%=turma.getCodigoTurma()%>">Excluir</a>
                            -
                            <a href="alteracidade.jsp?codigo=<%=turma.getCodigoTurma()%>">Alterar</a>
                        </td>
                    </tr>
                <%
                    }        
